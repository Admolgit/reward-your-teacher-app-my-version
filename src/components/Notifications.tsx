    ...
    return (
        <div className="flex text-[14px] mb-[3%] p-[2%] shadow-md rounded justify-between">
            <div className="flex flex-col text">
                <div className="mb-[2%]"><span className="font-[600]">{props.date}</span>, {props.time}</div>
                <div className="font-[400] text-[#21334F]">{props.message}</div>
            </div>
            {
                props.status ? (<div className="text-center w-[10%]">
                    <div className="rounded-full text-[#34A853] text-center py-[5%] mb-[2%] bg-lime-100">Status</div>
                    <div>Claimed</div>
                </div>)
                : null
            }
        ...
        <div className="sm:ml-[40%] sm-[740px]:ml-[42%] sm-[700px]:ml-[44%] ml-[30%] font-inter mt-[3%] w-[100%] mr-[10%]">
            ...
            <hr className="mb-[5%]" />
            {
                dummyData.map((items: any, key: any) => {
                    return <Notify index={key} date={items.date} time={items.time} message={items.message} status={items.status} />
                })
            }
        </div>
    )
